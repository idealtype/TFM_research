#!/usr/bin/env python3
"""Evaluate a soft-mask h96 decoder autoregressively on real LOTSA + ETT datasets."""
...
def load_ar_h96_model(checkpoint_root: Path, device: torch.device) -> tuple[FuncDecModel, Path, dict]:
    found = next((path for path in checkpoint_candidates(checkpoint_root, AR_HORIZON) if path.exists()), None)
    if found is None:
        tried = "\n".join(str(path) for path in checkpoint_candidates(checkpoint_root, AR_HORIZON))
        raise FileNotFoundError(f"No h{AR_HORIZON} checkpoint for AR eval. Tried:\n{tried}")

    state, ckpt_cfg = load_checkpoint_payload(found, device)
    cfg = build_config(AR_HORIZON, {}, ckpt_cfg)
    ...
    missing = [
        ...
        and not key.startswith("decoder_s.gating_network.")
        and not key.startswith("decoder_s.mlp_monthly.")
        and not key.startswith("decoder_s.forecast_head_monthly.")
    ]
...
@torch.no_grad()
def forecast_ar_h96(model: FuncDecModel, contexts_raw: np.ndarray, target_horizon: int,
                    freq: str, init_mu: torch.Tensor, init_sigma: torch.Tensor,
                    device: torch.device) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    context = torch.as_tensor(contexts_raw, dtype=torch.float32, device=device)
    masks = torch.zeros_like(context, dtype=torch.bool, device=device)
    init_mu = init_mu.to(device)
    init_sigma = init_sigma.to(device)
    init_denom = torch.where(init_sigma >= 1e-3, init_sigma, torch.ones_like(init_sigma))
    freq_key = freq if freq in FREQ_DAYS else "hourly"
    bases = build_soft_mask_basis(freq_key, AR_HORIZON)
    steps = int(np.ceil(int(target_horizon) / AR_HORIZON))
    ...
    for _ in range(steps):
        ...
        pred_n, decomp = model(emb, daily, weekly, monthly, yearly)
        no_res_n = decomp["trend"] + decomp["seasonal"]

        pred_raw = pred_n * denom + mu
        no_res_raw = no_res_n * denom + mu
        residual_raw = decomp["residual"] * denom
        trend_raw = decomp["trend"] * denom + mu
        seasonal_raw = decomp["seasonal"] * denom

        pred_raw_blocks.append(pred_raw)
        no_res_raw_blocks.append(no_res_raw)
        residual_raw_blocks.append(residual_raw)
        trend_raw_blocks.append(trend_raw)
        seasonal_raw_blocks.append(seasonal_raw)
        context = torch.cat([context[:, AR_HORIZON:], pred_raw], dim=1)
        masks = torch.zeros_like(context, dtype=torch.bool, device=device)

    pred_raw_all = torch.cat(pred_raw_blocks, dim=1)[:, :target_horizon]
    decomp_raw = {
        "no_residual": torch.cat(no_res_raw_blocks, dim=1)[:, :target_horizon],
        "residual": torch.cat(residual_raw_blocks, dim=1)[:, :target_horizon],
        "trend": torch.cat(trend_raw_blocks, dim=1)[:, :target_horizon],
        "seasonal": torch.cat(seasonal_raw_blocks, dim=1)[:, :target_horizon],
    }
    pred_init_n = (pred_raw_all - init_mu) / init_denom
    decomp_init_n = {
        "no_residual": (decomp_raw["no_residual"] - init_mu) / init_denom,
        "residual": decomp_raw["residual"] / init_denom,
        "trend": (decomp_raw["trend"] - init_mu) / init_denom,
        "seasonal": decomp_raw["seasonal"] / init_denom,
    }
    return pred_init_n, decomp_init_n
...
def evaluate_dataset(model, tfm_model, dataset: FineMaskRealDataset, batch_size: int,
    ...
    for batch in loader:
        ...
        if contexts_np is None:
            continue
        init_mu = dataset.mu.index_select(0, batch["source_idx"]).to(device)
        init_sigma = dataset.sigma.index_select(0, batch["source_idx"]).to(device)
        pred, decomp = forecast_ar_h96(
            model, contexts_np, dataset.horizon, dataset.freq, init_mu, init_sigma, device
        )
        add_error(acc, pred, future_n_dev)
        no_residual_pred = decomp["no_residual"]
    ...
    if acc["n"] == 0:
        raise ValueError(f"No raw contexts available for AR evaluation: {dataset.cache_dir} h{dataset.horizon}")
...
def run(args: argparse.Namespace) -> None:
    ...
    model, ckpt_path, _cfg = load_ar_h96_model(args.checkpoint_root, device)
    log_progress(f"loaded AR base h{AR_HORIZON} checkpoint {ckpt_path}")

    for horizon in args.horizons:
        ckpt_by_horizon[str(horizon)] = str(ckpt_path)
        log_progress(f"h{horizon}: evaluating autoregressive h{AR_HORIZON} decoder")
        tfm_model = None
    ...
    plot_model_vs_tfm_by_horizon(rows, out_root / "performance_by_horizon_all.png",
                                  "Soft-mask AR h96 real eval MAE by horizon")
