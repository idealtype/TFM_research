...
Differences from fine_mask:
  ...
  - Fourier warmup supervises gated effective seasonal coefficients directly.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fourier_synth_seasonal_loss",
                        ...
                        help="Seasonal target used for Fourier synthetic training. "
                             "coeff_effective compares (raw coefficients * gates) "
                             "to cached coefficient targets.")
...
class FourierSynthPayloadCache:
    ...
    def load(cls, group: dict) -> dict:
        ...
        coeff_targets = load_seasonal_coeff_targets(ds_dir, horizon)
        if coeff_targets is not None:
            for tensor in coeff_targets.values():
                finite = finite & torch.isfinite(tensor).all(dim=1)
...
def find_seasonal_coeff_path(ds_dir: Path, horizon: int) -> Path | None:
    candidates = [
...
def _extract_coeff_family(payload, family: str) -> torch.Tensor:
    if isinstance(payload, dict):
        ...
        for nested_key in ("seasonal_coefficients", "coefficients", "coeffs"):
            ...
            if isinstance(nested, dict):
                return _extract_coeff_family(nested, family)
    raise KeyError(f"missing {family} coefficient target")


def load_seasonal_coeff_targets(ds_dir: Path, horizon: int) -> dict[str, torch.Tensor] | None:
    path = find_seasonal_coeff_path(ds_dir, horizon)
    if path is None:
        return None
    raw = torch.load(path, map_location="cpu", weights_only=False)
    targets = {}
    for family in COEFF_FAMILIES:
        value = _extract_coeff_family(raw, family).float()
        if value.ndim != 2:
            raise ValueError(f"{path}: {family} coefficients must be rank-2, got {tuple(value.shape)}")
        expected_width = 2 * K_MAX[family]
        if value.shape[1] != expected_width:
            raise ValueError(
                f"{path}: {family} coefficients width={value.shape[1]}, expected {expected_width}"
            )
        targets[family] = value
    return targets


def sample_fourier_batch(payload: dict, batch_size: int, seed: int, step: int,
                         device: torch.device, return_coeff_targets: bool = False):
    ...
    coeff_targets = None
    if return_coeff_targets:
        if payload.get("coeff_targets") is None:
            raise ValueError("Fourier coefficient targets are required for coeff_effective synthetic loss")
        coeff_targets = {
            family: tensor.index_select(0, chosen_t).to(device, non_blocking=True)
            for family, tensor in payload["coeff_targets"].items()
        }
    bases = payload["bases"]
    daily, weekly, monthly, yearly = expand_bases(bases, emb.shape[0], device)
    if return_coeff_targets:
        return emb, future_n, trend_n, seasonal_n, coeff_targets, daily, weekly, monthly, yearly
    return emb, future_n, trend_n, seasonal_n, daily, weekly, monthly, yearly


def gated_effective_coefficients(decomp: dict) -> dict[str, torch.Tensor]:
    raw_coeffs = decomp["seasonal_coefficients"]
    gates = decomp["gates"]
    effective = {}
    for family in COEFF_FAMILIES:
        start, end = GATE_SLICES[family]
        gate = gates[:, start:end].repeat_interleave(2, dim=1)
        effective[family] = raw_coeffs[family] * gate
    return effective


def coefficient_loss(decomp: dict, coeff_targets: dict[str, torch.Tensor]) -> torch.Tensor:
    effective = gated_effective_coefficients(decomp)
    losses = [F.l1_loss(effective[family], coeff_targets[family]) for family in COEFF_FAMILIES]
...
def train_fourier_synth(model: FuncDecModel, groups: list[dict], steps: int,
    ...
    seasonal_loss_mode = getattr(args, "fourier_synth_seasonal_loss", None)
    if seasonal_loss_mode is None:
        seasonal_loss_mode = getattr(args, "fourier_warmup_seasonal_loss", None) or "seasonal_ts"
    ...
    for step in range(1, int(steps) + 1):
        ...
        if seasonal_loss_mode == "coeff_effective":
            emb, future_n, trend_n, seasonal_n, coeff_targets, daily, weekly, monthly, yearly = sample_fourier_batch(
        ...
        if seasonal_loss_mode == "coeff_effective":
            ...
            coeff_loss = coefficient_loss(decomp, coeff_targets)
        ...
        gate_l1 = args.gate_l1_weight * decomp["gates"].mean()
        loss = loss + gate_l1
        ...
        if step == 1 or step % args.log_every == 0 or step == int(steps):
            ...
            row = {
                ...
                "seasonal_loss_mode": seasonal_loss_mode,
            }
            ...
            history.append(row)
            extra = ""
            if "trend_loss" in row:
                extra += f" trend={row['trend_loss']:.6g}"
            if "coeff_loss" in row:
                extra += f" coeff={row['coeff_loss']:.6g}"
            if "seasonal_ts_loss" in row:
                extra += f" seasonal_ts={row['seasonal_ts_loss']:.6g}"
            print(
                f"[fourier] h{horizon} step {step}/{steps} "
                f"mode={seasonal_loss_mode} loss={value:.6g}"
                f"{extra} gate_l1={row['gate_l1']:.6g}",
                flush=True,
            )
