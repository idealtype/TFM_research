#!/usr/bin/env bash
# Soft-mask Fourier coefficient-loss warm-start only + real-dominant mixed training/evaluation.
set -euo pipefail

DEVICE=${DEVICE:-cuda:0}
PROJECT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RESULTS=${RESULTS_ROOT:-$PROJECT/results/residual_head_tfm_style_warm_real_mix_synth13_b1024_parallel_coeff_warmup_only}
...
run_horizon() {
  ...
  DEVICE=$DEVICE python "$PROJECT/train_warm_real_mix.py" \
    ...
    --fourier_synth_seasonal_loss coeff_effective \
...
if data.get("args", {}).get("fourier_synth_seasonal_loss") != "coeff_effective":
    bad.append(("fourier_synth_seasonal_loss", data.get("args", {}).get("fourier_synth_seasonal_loss")))
...
print(f"[verify] scratch + coeff_effective + batch_size=1024 + synth_interval=10 confirmed: {path}")
PY
