# Decoder Adjustment + Coefficient-Loss Warmup And Mixed Synth
...
- Change only the Fourier synthetic seasonal loss:
  - previous: `L1(decomp["seasonal"], seasonal_n)`
  - new: `L1(raw_coefficients * gates, cached_seasonal_coefficients)`

## Important Notes

- The coefficient target is used in both the initial `fourier_warmup` phase and sparse `mixed_synth` batches inside `mixed_full`.
- Real batches and residual-only finetuning keep the original losses.
- The historical `oldloss` name refers to the recorded result label. This folder uses `trend_loss + coeff_effective_loss + gate_l1` for Fourier synthetic batches, not the older erroneous `pred_loss + component_loss` style.
