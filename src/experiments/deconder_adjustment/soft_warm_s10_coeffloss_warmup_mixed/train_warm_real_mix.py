#!/usr/bin/env python3
"""Fourier-warm + real-dominant mixed training for soft_mask.
...
DEFAULT_RESULTS_ROOT = Path(__file__).resolve().parent / "results" / "residual_head_tfm_style_warm_real_mix_scratch"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fourier_synth_seasonal_loss",
                        choices=["seasonal_ts", "coeff_effective"],
                        default="coeff_effective",
                        help="Seasonal target used for both Fourier warmup and mixed_synth batches.")
    parser.add_argument("--fourier_warmup_seasonal_loss",
                        choices=["seasonal_ts", "coeff_effective"],
                        default=None,
                        help="Backward-compatible alias for --fourier_synth_seasonal_loss.")
    parser.add_argument("--mixed_steps", type=int, default=10000)
    parser.add_argument("--residual_steps", type=int, default=2000)
    ...
    if args.fourier_warmup_seasonal_loss is not None:
        args.fourier_synth_seasonal_loss = args.fourier_warmup_seasonal_loss
    return args
    ...
    for step in range(1, int(steps) + 1):
        ...
        if use_synth:
            ...
            try:
                ...
                continue
            if args.fourier_synth_seasonal_loss == "coeff_effective":
                emb, future_n, trend_n, seasonal_n, coeff_targets, daily, weekly, monthly, yearly = base.sample_fourier_batch(
                    payload,
                    args.batch_size,
                    args.seed + horizon * 19,
                    step,
                    device,
                    return_coeff_targets=True,
                )
            else:
                coeff_targets = None
            ...
            trend_loss = None
            coeff_loss = None
            seasonal_ts_loss = None
            if trend_n is not None:
                trend_loss = F.l1_loss(decomp["trend"], trend_n)
                loss_terms.append(trend_loss)
            if args.fourier_synth_seasonal_loss == "coeff_effective":
                if coeff_targets is None:
                    raise RuntimeError("coeff_effective selected without coefficient targets")
                coeff_loss = base.coefficient_loss(decomp, coeff_targets)
                loss_terms.append(coeff_loss)
                if seasonal_n is not None:
                    seasonal_ts_loss = F.l1_loss(decomp["seasonal"], seasonal_n).detach()
            elif seasonal_n is not None:
                seasonal_ts_loss = F.l1_loss(decomp["seasonal"], seasonal_n)
                loss_terms.append(seasonal_ts_loss)
        ...
        if should_log:
            ...
            if use_synth:
                row["seasonal_loss_mode"] = args.fourier_synth_seasonal_loss
                if trend_loss is not None:
                    row["trend_loss"] = float(trend_loss.item())
                if coeff_loss is not None:
                    row["coeff_loss"] = float(coeff_loss.item())
                if seasonal_ts_loss is not None:
                    row["seasonal_ts_loss"] = float(seasonal_ts_loss.item())
            history.append(row)
            synth_extra = ""
            if use_synth:
                synth_extra = f" mode={row['seasonal_loss_mode']}"
                if "coeff_loss" in row:
                    synth_extra += f" coeff={row['coeff_loss']:.6g}"
                if "seasonal_ts_loss" in row:
                    synth_extra += f" seasonal_ts={row['seasonal_ts_loss']:.6g}"
            print(
                ...
                f"ts_corr={row['trend_seasonal_corr']:.6g}{synth_extra}",
                flush=True,
            )
...
def main() -> None:
    ...
    print(
        ...
        f"synth_interval={args.synth_interval} "
        f"fourier_synth_seasonal_loss={args.fourier_synth_seasonal_loss} "
