#!/usr/bin/env python3
"""Parallel horizon runner for soft_mask real evaluation.

Runs each horizon in a separate non-daemon process sharing the same GPU.
mp.Pool workers are daemon processes and cannot spawn DataLoader child
processes; mp.Process is used directly to avoid this restriction.
...
Each batch of N horizons runs in parallel; remaining horizons run in the
next batch.
"""
...
def _eval_horizon_worker(
    ...
    try:
        ...
        rows, ckpt_path_str = [], ""
    queue.put((horizon, rows, ckpt_path_str))
...
def main() -> None:
    ...
    all_results: list[tuple[int, list[dict], str]] = []

    # Process horizons in batches of n_parallel
    for batch_start in range(0, len(horizons), n_parallel):
        batch = horizons[batch_start: batch_start + n_parallel]
        processes = [
            mp.Process(
                target=_eval_horizon_worker,
                args=(queue, h, args, manifest, device_str),
            )
            for h in batch
        ]
        for p in processes:
            p.start()
        all_results.extend(queue.get() for _ in batch)
        for p in processes:
            p.join()

    rows = []
    ckpt_by_horizon = {}
    for horizon, horizon_rows, ckpt_path_str in sorted(all_results, key=lambda x: x[0]):
