...
# with trend-delta sparsity and trend-seasonal orthogonal penalties.
set -euo pipefail

DEVICE=${DEVICE:-cuda:0}
PROJECT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RESULTS=${RESULTS_ROOT:-$PROJECT/results/residual_head_tfm_style_warm_real_mix_synth13_b1024_parallel_trend_sparse_orthogonal}
...
TS_CORR_WEIGHT=${TS_CORR_WEIGHT:-0.01}
TREND_DELTA_L1_WEIGHT=${TREND_DELTA_L1_WEIGHT:-0.2}
...
echo "TS_CORR_WEIGHT=$TS_CORR_WEIGHT TREND_DELTA_L1_WEIGHT=$TREND_DELTA_L1_WEIGHT"
...
run_horizon() {
  ...
  DEVICE=$DEVICE python "$PROJECT/train_warm_real_mix.py" \
    ...
    --ts_corr_weight "$TS_CORR_WEIGHT" \
    --gate_l1_weight 1e-3 \
    --trend_delta_l1_weight "$TREND_DELTA_L1_WEIGHT" 2>&1 | sed "s/^/[train h${horizon}] /"
}
