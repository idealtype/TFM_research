...
Differences from fine_mask:
  ...
  - Loss includes gate L1 sparsity and trend changepoint delta L1 sparsity
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--trend_delta_l1_weight", type=float, default=0.2,
                        help="Weight for trend changepoint delta L1 sparsity regularization.")
...
def train_fourier_synth(model: FuncDecModel, groups: list[dict], steps: int,
    ...
    for step in range(1, int(steps) + 1):
        ...
        loss = (
            ...
            + args.trend_delta_l1_weight * trend_sparse
        )
        ...
        if step == 1 or step % args.log_every == 0 or step == int(steps):
            ...
            trend_sparse_value = float(trend_sparse.item())
            history.append({
                "phase": "fourier_synth",
                "step": step,
                "loss": value,
                "trend_delta_l1": trend_sparse_value,
            })
            print(
                f"[fourier] h{horizon} step {step}/{steps} "
                f"loss={value:.6g} trend_delta_l1={trend_sparse_value:.6g}",
                flush=True,
            )
...
def train_nonf(model: FuncDecModel, items: list[tuple[Path, dict]], steps: int,
    ...
    for batch in loader:
        ...
        loss = (
            ...
            + args.gate_l1_weight * decomp["gates"].mean()
            + args.trend_delta_l1_weight * trend_delta_l1(decomp)
        )
        ...
        if step == 1 or step % args.log_every == 0 or step == int(steps):
            value = float(loss.item())
            trend_sparse_value = float(trend_delta_l1(decomp).item())
            history.append({
                "phase": "nonf",
                "step": step,
                "loss": value,
            ...
            print(
                f"[nonf] h{horizon} step {step}/{steps} "
                f"loss={value:.6g} trend_delta_l1={trend_sparse_value:.6g}",
                flush=True,
            )
        if step >= int(steps):
    ...
    for step in range(1, int(steps) + 1):
        ...
        if step == 1 or step % args.log_every == 0 or step == int(steps):
            ...
            print(
                ...
                f"trend_delta_l1={float(trend_sparse.item()):.6g}",
                flush=True,
            )
