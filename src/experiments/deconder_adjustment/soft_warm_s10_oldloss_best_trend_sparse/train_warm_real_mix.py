...
Trend changepoint delta L1 sparsity is applied during full-decoder warmup and
mixed training, but not during residual-only finetuning.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--trend_delta_l1_weight", type=float, default=0.2,
                        help="Trend changepoint delta L1 sparsity regularization.")
...
def trend_delta_l1(decomp: dict[str, torch.Tensor]) -> torch.Tensor:
    return decomp["delta"].abs().mean()
...
def train_mixed_full(model: FuncDecModel, real_groups: list[dict], synth_groups: list[dict],
    ...
    for step in range(1, int(steps) + 1):
        ...
        if use_synth:
            ...
            trend_sparse = trend_delta_l1(decomp)
            loss = (
                loss
                + float(args.gate_l1_weight) * decomp["gates"].mean()
                + float(args.trend_delta_l1_weight) * trend_sparse
            )
        ...
        if should_log:
            ...
            row = {
                ...
                "trend_delta_l1": float(trend_sparse.item()),
            }
            history.append(row)
            print(
                ...
                f"trend_delta_l1={row['trend_delta_l1']:.6g}",
                flush=True,
            )
...
def main() -> None:
    ...
    print(
        ...
        f"trend_delta_l1_weight={args.trend_delta_l1_weight}",
        flush=True,
    )
