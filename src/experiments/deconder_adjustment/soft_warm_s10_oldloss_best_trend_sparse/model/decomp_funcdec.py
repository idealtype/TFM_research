...
class FuncDecModel(nn.Module):
    """FuncDec model with soft harmonic gating in the seasonal decoder.

    Identical to fine_mask FuncDecModel except:
    - SeasonalDecoder now includes HarmonicGatingNetwork
    - forward() returns "gates" key in decomp dict for L1 loss
    - forward() signature unchanged: (emb, daily, weekly, monthly, yearly)
    """
    ...
    def forward(self, emb, daily_basis, weekly_basis, monthly_basis, yearly_basis):
        ...
        # SeasonalDecoder returns (seasonal, gates) or (seasonal, coeffs, gates)
        seasonal_n, seasonal_coefficients, gates = self.decoder_s(
            emb,
            daily_basis,
            weekly_basis,
            monthly_basis,
        ...
        return pred_n, {
            ...
            "gates": gates,   # (B, 24) — used for L1 sparsity loss in train.py
        }

    def _encode(self, context, masks):
        if self.backbone is None:
            raise NotImplementedError("load_backbone=False. 추론용 별도 스크립트 사용.")
        batch_size, context_len = context.shape
        if context_len != self.context_len:
            raise ValueError(f"context length mismatch: {context_len} != {self.context_len}")
        if context_len % self.patch_len != 0:
            raise ValueError(f"context_len={context_len} not divisible by patch_len={self.patch_len}")
