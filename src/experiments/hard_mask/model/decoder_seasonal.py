...
# This unifies HEAD capacity and actual order unlike the previous version.
...
class SeasonalDecoder(nn.Module):
    ...
    def predict_coefficients(self, emb):
        ...
        coef_m = self.forecast_head_monthly(self.mlp_monthly(emb))
        coef_y = self.forecast_head_yearly(self.mlp_yearly(emb))
        return {"daily": coef_d, "weekly": coef_w, "monthly": coef_m, "yearly": coef_y}

    def forward(self, emb, daily_basis, weekly_basis, monthly_basis, yearly_basis,
                return_coefficients=False):
        coeffs = self.predict_coefficients(emb)
        coef_d = coeffs["daily"]
        coef_w = coeffs["weekly"]
        coef_m = coeffs["monthly"]
        coef_y = coeffs["yearly"]
