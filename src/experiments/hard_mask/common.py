...
MODEL_NAME = os.environ.get("FUNCDEC_MODEL_NAME", "fine_mask")
...
def adapt_state_dict_for_fine_mask(state: dict, model: FuncDecModel) -> dict:
    """Adapt a legacy state dict to the fine_mask model.

    Handles:
    - Size mismatch for weekly/yearly forecast_head (old: 20 out, new: 8/16)
      by truncating the loaded weights to the new model size.
    - Missing monthly keys are kept absent (will be randomly initialized).
    - Unexpected backbone keys are skipped.
    """
    model_state = model.state_dict()
    adapted = {}
    for key, param in state.items():
        if key.startswith("backbone."):
            continue  # skip backbone keys
        if key not in model_state:
            continue  # skip unknown keys (e.g., monthly doesn't exist in old ckpts)
        target_shape = model_state[key].shape
        if param.shape == target_shape:
            adapted[key] = param
        elif param.ndim == len(target_shape):
            # Size mismatch with same number of dimensions: truncate or pad
            # e.g., forecast_head_weekly.weight: [20, 1280] -> [8, 1280]
            ...
            if all(param.shape[i] >= target_shape[i] for i in range(len(target_shape))):
                ...
                # Source is smaller in some dim: pad with zeros
                t = torch.zeros(target_shape, dtype=param.dtype)
                src_slices = tuple(slice(0, min(param.shape[i], target_shape[i])) for i in range(len(target_shape)))
...
def load_single_model(run_dir: Path, horizon: int, device: torch.device) -> tuple[FuncDecModel, Path, dict]:
    """Load a fine_mask model checkpoint. Uses strict=False + weight adaptation for legacy checkpoints."""
    ...
    # monthly-related missing keys are expected when loading from pre-monthly checkpoints
    missing = [key for key in incompatible.missing_keys
               if key.startswith("decoder_") and "monthly" not in key]
    ...
    if monthly_missing:
        print(f"  [info] monthly weights randomly initialized: {len(monthly_missing)} keys", flush=True)
    model.eval()
