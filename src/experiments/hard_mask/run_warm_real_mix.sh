#!/usr/bin/env bash
# Fine-mask Fourier warm-start + real-dominant mixed training/evaluation.
set -euo pipefail

DEVICE=${DEVICE:-cuda:0}
PROJECT=/home/sia2/project/5.30fine_mask
RESULTS=${RESULTS_ROOT:-$PROJECT/results/fourier_warm_real_mix_scratch_synth13_b1024_parallel_trend_seasonal_loss}
...
echo "=== fine-mask warm-real-mix pipeline start $(date '+%Y-%m-%d %H:%M:%S') ==="
echo "DEVICE=$DEVICE"
...
run_horizon() {
  ...
  DEVICE=$DEVICE python "$PROJECT/train_warm_real_mix.py" \
    ...
    --ts_corr_weight 0.01 2>&1 | sed "s/^/[train h${horizon}] /"
}
...
print(f"[verify] scratch + batch_size=1024 + synth_interval=13 confirmed: {path}")
PY
...
# Synthetic evaluation is intentionally disabled for the current fine-mask runs.
...
echo "=== fine-mask warm-real-mix pipeline complete $(date '+%Y-%m-%d %H:%M:%S') ==="
