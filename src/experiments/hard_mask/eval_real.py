#!/usr/bin/env python3
"""Evaluate fine_mask model on real LOTSA + ETT datasets."""
...
class FineMaskRealDataset(Dataset):
    def __init__(self, cache_dir: Path, horizon: int, samples_per_dataset: int, seed: int,
        ...
        # Resolve basis: prefer fine_mask, fallback to legacy upgrade, then on-the-fly
        fine_mask_files = sorted(cache_dir.glob(
            f"fourier_basis_c{self.context_len}_{self.freq}_h{self.horizon}_fine_mask*.pt"
        ))
        if fine_mask_files:
            self.bases = load_fine_mask_basis(fine_mask_files[0])
        else:
            legacy_files = sorted(cache_dir.glob(
                f"fourier_basis_c{self.context_len}_{self.freq}_h{self.horizon}_*.pt"
            ))
            if legacy_files:
                self.bases = upgrade_legacy_basis(legacy_files[0])
            else:
                freq_key = self.freq if self.freq in FREQ_DAYS else "hourly"
                self.bases = build_fine_mask_basis(freq_key, self.context_len, self.horizon)
...
def run(args: argparse.Namespace) -> None:
    ...
    plot_model_vs_tfm_by_horizon(rows, out_root / "performance_by_horizon_all.png",
                                  "Fine-mask real eval MAE by horizon")
