...
from covariates import batch_raw_dataframe_covariates, parse_column_list  # noqa: E402
from xreg import fit_linear_xreg  # noqa: E402
try:  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--xreg_ridge", type=float, default=0.0,
                        help="Ridge penalty for in-context XReg. TimesFM default is 0.0.")
    parser.add_argument("--xreg_covariate_columns", type=str, default=None,
                        help="Comma-separated covariate columns or a text file. Default: all numeric non-target raw columns.")
    parser.add_argument("--xreg_exclude_columns", type=str, default=None,
                        help="Comma-separated covariate columns to exclude or a text file.")
    parser.add_argument("--no_xreg_normalize_target", dest="xreg_normalize_target", action="store_false",
                        help="Disable per-series target normalization before XReg fitting.")
    parser.set_defaults(xreg_normalize_target=True)
...
def load_tfm_backcast_model(context_len: int, horizon: int, hf_cache_dir: str | None = None):
...
def evaluate_dataset(model, tfm_model, dataset: FineMaskRealDataset, batch_size: int,
    ...
    plot_items = []
    xreg_status_counts: dict[str, int] = {}
    xreg_feature_sum = 0.0
    xreg_feature_n = 0
    include_columns = parse_column_list(args.xreg_covariate_columns)
    exclude_columns = parse_column_list(args.xreg_exclude_columns)
    ...
    for batch in loader:
        ...
        xreg_pred_n = None
        xreg_component_n = None
        xreg_status_by_pos = ["not_run"] * int(pred.shape[0])
        xreg_feature_count_by_pos = [0] * int(pred.shape[0])
        xreg_covariates_by_pos = [[] for _ in range(int(pred.shape[0]))]
        xreg_backcast_point = None
        if tfm_model is not None:
            xreg_pred_n = pred.detach().clone()
            xreg_component_n = torch.zeros_like(pred.detach())
            xreg_status_by_pos = ["no_covariates"] * int(pred.shape[0])
            raw_df = dataset._load_raw_df()
            windows = batch_raw_dataframe_covariates(
                raw_df=raw_df,
                backbone=dataset.backbone,
                local_indices=batch["source_idx"],
                context_len=dataset.context_len,
                horizon=dataset.horizon,
                include_columns=include_columns,
                exclude_columns=exclude_columns,
            )
            valid_positions = [i for i, window in enumerate(windows) if window is not None]
            if valid_positions:
                context_inputs = [windows[i].target_context for i in valid_positions]  # type: ignore[union-attr]
                xreg_backcast_point, _ = tfm_model.forecast(dataset.horizon, context_inputs)
                xreg_backcast_point = np.asarray(xreg_backcast_point, dtype=np.float32)
                backcast = xreg_backcast_point[:, :-dataset.horizon]
                ...
                for out_pos, batch_pos in enumerate(valid_positions):
                    window = windows[batch_pos]
                    assert window is not None
                    xreg_covariates_by_pos[batch_pos] = list(window.covariate_names)
                    train_len = min(
                        int(backcast.shape[1]),
                        int(window.target_context.shape[0]),
                        int(window.covariates_context.shape[0]),
                    )
                    if train_len <= 0:
                        status = "no_backcast"
                        xreg_status_by_pos[batch_pos] = status
                        xreg_status_counts[status] = xreg_status_counts.get(status, 0) + 1
                        continue
                    target = window.target_context[-train_len:] - backcast[out_pos, -train_len:]
                    result = fit_linear_xreg(
                        target,
                        window.covariates_context[-train_len:],
                        window.covariates_future,
                        ridge=float(args.xreg_ridge),
                        normalize_target=bool(args.xreg_normalize_target),
                    )
                    xreg_status_by_pos[batch_pos] = result.status
                    xreg_feature_count_by_pos[batch_pos] = int(result.n_features)
                    xreg_status_counts[result.status] = xreg_status_counts.get(result.status, 0) + 1
                    xreg_feature_sum += float(result.n_features)
                    xreg_feature_n += 1
                    if result.status == "ok":
                        xreg_raw = torch.as_tensor(result.forecast, dtype=torch.float32, device=device)
                        contribution_n = xreg_raw / denom[batch_pos].clamp_min(1e-8)
                        xreg_component_n[batch_pos] = contribution_n
                        xreg_pred_n[batch_pos] = pred[batch_pos].detach() + contribution_n
                invalid_count = int(pred.shape[0]) - len(valid_positions)
                if invalid_count > 0:
                    status = "no_covariates"
                    xreg_status_counts[status] = xreg_status_counts.get(status, 0) + invalid_count
            else:
                status = "no_covariates"
                xreg_status_counts[status] = xreg_status_counts.get(status, 0) + int(pred.shape[0])
            add_error(xreg_acc, xreg_pred_n, future_n_dev)

            contexts_np = dataset.raw_contexts_for_local_indices(batch["source_idx"])
            if contexts_np is not None:
                if xreg_backcast_point is not None and len(valid_positions) == len(windows):
                    point_forecast = xreg_backcast_point[:, -dataset.horizon:]
                else:
                    point_with_backcast, _ = tfm_model.forecast(dataset.horizon, [x for x in contexts_np])
                    point_forecast = np.asarray(point_with_backcast, dtype=np.float32)[:, -dataset.horizon:]
        ...
        if len(plot_items) < plot_limit:
            ...
            for i in range(take):
                plot_items.append({
                    ...
                    "xreg_pred": None if xreg_pred_n is None else xreg_pred_n[i].detach().cpu(),
                    "xreg_component": None if xreg_component_n is None else xreg_component_n[i].detach().cpu(),
                    "xreg_status": xreg_status_by_pos[i],
                    "xreg_n_features": xreg_feature_count_by_pos[i],
                    "xreg_covariates": xreg_covariates_by_pos[i],
    ...
    metrics = {
        ...
        "xreg_total_mae": None if xreg_acc["n"] == 0 else finalize_mae(xreg_acc),
        "xreg_total_mse": None if xreg_acc["n"] == 0 else finalize_mse(xreg_acc),
        "xreg_delta_mae": None if xreg_acc["n"] == 0 else total_mae - finalize_mae(xreg_acc),
        "xreg_status": ",".join(f"{k}:{v}" for k, v in sorted(xreg_status_counts.items())) or None,
        "xreg_mean_features": None if xreg_feature_n == 0 else xreg_feature_sum / xreg_feature_n,
...
EVAL_FIELDNAMES = [
    ...
    "total_mae", "total_mse",
    "xreg_total_mae", "xreg_total_mse", "xreg_delta_mae", "xreg_status", "xreg_mean_features",
    "tfm_zeroshot_mae", "tfm_zeroshot_mse",
...
def eval_horizon(
    ...
    if not args.skip_tfm:
        log_progress(f"h{horizon}: loading TimesFM with return_backcast=True for XReg")
        tfm_model = load_tfm_backcast_model(512, int(horizon), args.hf_cache_dir)
    ...
    for item_idx, item in enumerate(manifest):
        ...
        row = {
            ...
            "xreg_total_mae": metrics["xreg_total_mae"],
            "xreg_total_mse": metrics["xreg_total_mse"],
            "xreg_delta_mae": metrics["xreg_delta_mae"],
            "xreg_status": metrics["xreg_status"],
            "xreg_mean_features": metrics["xreg_mean_features"],
        ...
        rows.append(row)
        xreg_msg = (
            "none" if metrics["xreg_total_mae"] is None
            else f"{metrics['xreg_total_mae']:.6g} delta={metrics['xreg_delta_mae']:.6g}"
        )
        log_progress(f"  done model_mae={metrics['total_mae']:.6g} xreg_mae={xreg_msg}")
