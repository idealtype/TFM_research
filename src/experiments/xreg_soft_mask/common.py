...
def plot_real_comparison_grid(path: Path, title: str, items: list[dict]) -> None:
    if not items:
        return
    shown = items[:3]
    fig, axes = plt.subplots(len(shown), 4, figsize=(22.0, 3.6 * len(shown)), squeeze=False)
    family_colors = {
        "daily": "#1f77b4",
        "weekly": "#ff7f0e",
        "monthly": "#9467bd",
    ...
    for row_idx, item in enumerate(shown):
        ...
        if item.get("xreg_pred") is not None:
            ax.plot(x, item["xreg_pred"], color="#17becf", label=f"{MODEL_NAME}+XReg")
        ...
        ax = axes[row_idx][2]
        xreg_component = item.get("xreg_component")
        if xreg_component is not None:
            ax.plot(x, xreg_component, color="#17becf", label="XReg contribution")
        else:
            ax.plot(x, np.zeros_like(x, dtype=np.float32), color="#17becf", alpha=0.35, label="XReg contribution")
        ax.axhline(0.0, color="0.65", linewidth=0.8, alpha=0.6)
        covariates = item.get("xreg_covariates") or []
        if covariates:
            shown_covariates = ", ".join(str(c) for c in covariates[:5])
            if len(covariates) > 5:
                shown_covariates += f", +{len(covariates) - 5}"
        else:
            shown_covariates = "none"
        ax.set_title(
            f"XReg contribution status={item.get('xreg_status', 'n/a')} "
            f"features={item.get('xreg_n_features', 0)}"
        )
        ax.text(
            0.01, 0.98, f"covariates: {shown_covariates}",
            transform=ax.transAxes,
            va="top",
            fontsize=7,
            bbox={"boxstyle": "round,pad=0.25", "facecolor": "white", "alpha": 0.78, "edgecolor": "0.8"},
