#!/usr/bin/env bash
# No-gate soft-mask Fourier warm-start + real-dominant mixed training/evaluation.
set -euo pipefail

DEVICE=${DEVICE:-cuda:0}
PROJECT=/home/sia2/project/6.1nogate_softmask
RESULTS=${RESULTS_ROOT:-$PROJECT/results/fourier_warm_real_mix_b1024_synth13_coeffl1}
...
echo "=== no-gate soft-mask warm-real-mix pipeline start $(date '+%Y-%m-%d %H:%M:%S') ==="
echo "DEVICE=$DEVICE"
...
echo "=== no-gate soft-mask warm-real-mix pipeline complete $(date '+%Y-%m-%d %H:%M:%S') ==="
