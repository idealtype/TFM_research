#!/usr/bin/env python3
"""Full training pipeline for 6.1nogate_softmask.
...
Differences from fine_mask:
  ...
  - Loss includes coefficient L1 sparsity: total_loss = recon + coeff_l1_weight * abs(coeff).mean()
...
def parse_args() -> argparse.Namespace:
    ...
    # Optimizer
    parser.add_argument("--coeff_l1_weight", type=float, default=1e-3,
                        help="Weight for seasonal coefficient L1 sparsity regularization (default: 1e-3). "
...
def load_initial_model(horizon: int, args: argparse.Namespace, device: torch.device):
    """Build model from scratch (nogate_softmask trains from random initialization)."""
    ...
    if init_dir != Path("none") and init_dir.exists():
        ...
        if found is not None:
            ...
            allowed_prefixes = ("decoder_s.mlp_monthly.", "decoder_s.forecast_head_monthly.")
...
def seasonal_coeff_l1(decomp: dict) -> torch.Tensor:
    coeffs = decomp["seasonal_coefficients"]
    return torch.stack([coef.abs().mean() for coef in coeffs.values()]).mean()
...
def resolve_basis_for_dir(ds_dir: Path, granularity: str, horizon: int, context_len: int) -> dict:
    """Compute nogate_softmask physics-only basis on-the-fly. Ignores cached basis files."""
...
def resolve_basis_for_real(group: dict) -> dict:
    """Compute nogate_softmask physics-only basis on-the-fly for real data."""
...
def train_fourier_synth(model: FuncDecModel, groups: list[dict], steps: int,
    ...
    for step in range(1, int(steps) + 1):
        ...
        loss = loss + args.coeff_l1_weight * seasonal_coeff_l1(decomp)
...
class NonFPayloadCache:
    ...
    def load(cls, ds_dir: Path, meta: dict) -> dict:
        ...
        # Compute nogate_softmask physics-only basis on-the-fly
        bases = resolve_basis_for_dir(ds_dir, granularity, horizon, context_len)
...
def train_nonf(model: FuncDecModel, items: list[tuple[Path, dict]], steps: int,
    ...
    for batch in loader:
        ...
        loss = (
            ...
            + args.coeff_l1_weight * seasonal_coeff_l1(decomp)
        )
...
def train_real(model: FuncDecModel, groups: list[dict], steps: int,
    ...
    for step in range(1, int(steps) + 1):
        ...
        pred, decomp = model(emb, daily, weekly, monthly, yearly)
        loss = F.l1_loss(pred, future_n) + args.coeff_l1_weight * seasonal_coeff_l1(decomp)
        optimizer.zero_grad(set_to_none=True)
        loss.backward()
        clip_grad_norm_(params, args.grad_clip)
        optimizer.step()
