#!/usr/bin/env python3
"""Fourier-warm + real-dominant mixed training for nogate_softmask.
...
DEFAULT_RESULTS_ROOT = base.resolve_project_path("/home/sia2/project/6.1nogate_softmask/results/fourier_warm_real_mix_b1024_synth13_coeffl1")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--coeff_l1_weight", type=float, default=1e-3,
                        help="Seasonal coefficient L1 sparsity regularization.")
...
def train_mixed_full(model: FuncDecModel, real_groups: list[dict], synth_groups: list[dict],
    ...
    for step in range(1, int(steps) + 1):
        ...
        if use_synth:
            ...
            loss = loss + float(args.coeff_l1_weight) * base.seasonal_coeff_l1(decomp)
            ...
            loss = (
                ...
                + float(args.coeff_l1_weight) * base.seasonal_coeff_l1(decomp)
            )
            subset = group["subset"]
