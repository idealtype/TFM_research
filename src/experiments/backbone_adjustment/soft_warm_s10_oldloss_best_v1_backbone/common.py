...
class TimesFmV1ForecastWrapper:
...
def load_tfm_zeroshot_model(context_len: int, horizon: int, hf_cache_dir: str | None = None):
    v1_src = os.environ.get("TIMESFM_V1_SRC")
    if v1_src:
        sys.path.insert(0, str(Path(v1_src).resolve()))
    for name in list(sys.modules):
        if name == "timesfm" or name.startswith("timesfm."):
            del sys.modules[name]
    from timesfm.timesfm_base import TimesFmCheckpoint, TimesFmHparams
    from timesfm.timesfm_torch import TimesFmTorch

    checkpoint_path = os.environ.get("TIMESFM_V1_CHECKPOINT_PATH")
    checkpoint = TimesFmCheckpoint(
        path=checkpoint_path if checkpoint_path and Path(checkpoint_path).exists() else None,
        huggingface_repo_id=None if checkpoint_path and Path(checkpoint_path).exists() else "google/timesfm-1.0-200m-pytorch",
        local_dir=hf_cache_dir,
    )
    model = TimesFmTorch(
        hparams=TimesFmHparams(
            context_len=int(context_len),
            horizon_len=int(math.ceil(max(int(horizon), 128) / 128) * 128),
            input_patch_len=32,
    ...
    model._model.eval()
    return TimesFmV1ForecastWrapper(model)
