...
DEFAULT_CHECKPOINT_ROOT = THIS_DIR / "results" / "train" / "domain_real_finetune"
DEFAULT_RESULTS_ROOT = THIS_DIR / "results"
MODEL_NAME = "domain_real_finetune"


def log_progress(message: str) -> None:
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [domain-real-eval] {message}", flush=True)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate target-specific real fine-tuned FuncDec checkpoints.")
...
def run(args: argparse.Namespace) -> None:
    ...
    for horizon in args.horizons:
        ...
        for item_idx, item in enumerate(manifest):
            ...
            if args.skip_tfm:
                metrics, plot_items = evaluate_dataset_without_tfm(
                    model, dataset, args.batch_size, device, args.plot_samples_per_dataset
                )
            else:
            ...
            tfm_msg = "None" if metrics["tfm_zeroshot_mae"] is None else f"{metrics['tfm_zeroshot_mae']:.6g}"
            log_progress(
                f"h{horizon}: done {dataset_name} n={len(dataset)} model_mae={metrics['total_mae']:.6g} tfm_mae={tfm_msg}"
            )
    ...
    plot_model_vs_tfm_by_horizon(rows, out_root / "performance_by_horizon_all.png", "Real eval overall MAE by horizon")
    write_summary(
