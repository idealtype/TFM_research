...
RESULT_ROOT="${EXP_DIR}/results/nosynthpretrain"
BASE_RESULT_ROOT="${EXP_DIR}/results/all_domain_full_then_residual"
...
echo "[1/7] train no-synth-pretrain all-domain full decoders then residual-only"
python "${EXP_DIR}/train_all_domain_real_then_residual.py" \
  ...
  2>&1 | tee "${LOG_DIR}/train_nosynthpretrain_full_then_residual.log"

echo "[2/7] evaluate real targets: full"
...
echo "[3/7] evaluate real targets: residual-extra"
python "${EXP_DIR}/eval_real_lot_ett_global_model.py" \
  --device "${DEVICE}" \
  --horizons ${HORIZONS} \
  --real_root "${REAL_ROOT}" \
...
echo "[4/7] evaluate F synthetic half groups: full and residual-extra"
...
echo "[5/7] evaluate non-F synthetic half groups: full"
...
echo "[6/7] evaluate non-F synthetic half groups: residual-extra"
python "${OLD_EXP_DIR}/eval_nonfourier_single_model.py" \
  --nonfourier_root "${NONF_SYNTH_ROOT}" \
  ...
  --batch_size "${EVAL_BATCH_SIZE}" \
  --device "${DEVICE}" \
  2>&1 | tee "${LOG_DIR}/eval_synth_nonF_residual_extra_half.log"

echo "[7/7] update shared comparison plots once"
python "${EXP_DIR}/plot_all_domain_comparisons.py" \
  --root "${BASE_RESULT_ROOT}" \
  --nosynth_root "${RESULT_ROOT}" \
  --out_dir "${BASE_RESULT_ROOT}/eval/comparison_plots" \
  2>&1 | tee "${LOG_DIR}/update_shared_comparison_plots.log"

echo "done"
echo "results: ${RESULT_ROOT}"
echo "updated plots: ${BASE_RESULT_ROOT}/eval/comparison_plots"
