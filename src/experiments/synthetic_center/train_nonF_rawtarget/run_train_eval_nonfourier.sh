...
echo "[train_nonF] exp_dir=${EXP_DIR}"
echo "[train_nonF] result_root=${RESULT_ROOT}"
echo "[train_nonF] run_name=${RUN_NAME}"
echo "[train_nonF] train_root=${TRAIN_ROOT}"
echo "[train_nonF] init_checkpoint_dir=${INIT_CHECKPOINT_DIR}"
echo "[train_nonF] residual distributions are distribution-noise caches, not structural AM residuals"
echo "[train_nonF] loss policy: trend->trend_n, seasonal+residual->seasonal_n, distribution noise excluded"
echo "[train_nonF] started_at=$(date '+%Y-%m-%d %H:%M:%S')"

python train_nonfourier_finetune.py \
...
echo "[train_nonF] evaluate non-Fourier synthetic"
...
echo "[train_nonF] evaluate real LOTSA/ETTh1"
